...
#elif LOCAL_LE
		sceDmacMemcpy(dst, src, GPU_FRAMEBUFFER_NATIVE_WIDTH * PIXELBYTES);
#else
		if (NEEDENDIANSWAP)
		{
...
#if defined(LOCAL_LE)
		sceDmacMemcpy(dst, src, lineWidth * lineCount * PIXELBYTES);
#else
		if (NEEDENDIANSWAP)
		{
...
GPUSubsystem::GPUSubsystem(void * screen = 0)
{
...
#define RGB15TO24_SWAP_RB_BITLOGIC(col) (  material_5bit_to_8bit[((col)>>10)&0x1F]      | (material_5bit_to_8bit[((col)>>5)&0x1F]<<8) | (material_5bit_to_8bit[(col)&0x1F]<<16) )
	...
	_displayInfo.masterNativeBuffer = screen;
...
GPUSubsystem* GPUSubsystem::Allocate(void * screen)
{
	return new(malloc_aligned64(sizeof(GPUSubsystem))) GPUSubsystem(screen); 
}
