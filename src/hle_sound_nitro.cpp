...
void StopChannel(int id, bool hold)
{
    ...
    u32 cnt = SPU_ReadLong(addr);
    cnt &= ~(1<<31);
    if (hold) cnt |= (1<<15);
    SPU_WriteLong(addr, cnt);
}
...
void SetupChannel_Wave(int id, u32 srcaddr, int format, int repeat, u16 looppos, u32 len, int vol, int voldiv, u16 freq, int pan)
{
    ...
    SPU_WriteLong(addr+0x00, cnt);
    SPU_WriteWord(addr+0x08, 0x10000-freq);
    SPU_WriteWord(addr+0x0A, looppos);
    SPU_WriteLong(addr+0x0C, len);
    SPU_WriteLong(addr+0x04, srcaddr);
}
...
void SetChannelVolume(int id, int vol, int voldiv)
{
    ...
    if ((SurroundDecay > 0) && (id != 1) && (id != 3))
    {
        int pan = SPU_ReadByte(addr+0x02);
        vol = CalcChannelVolume(vol, pan);
    }
...
void SetChannelPan(int id, int pan)
{
    ...
    SPU_WriteByte(addr+0x02, pan);

    if ((SurroundDecay > 0) && (id != 1) && (id != 3))
    {
        int vol = CalcChannelVolume(ChanVolume[id], pan);
        SPU_WriteByte(addr+0x00, vol);
    }
}
...
void UpdateHardwareChannels()
{
    ...
    for (int i = 0; i < 16; i++)
    {
        ...
        if (chan->StatusFlags & (1<<3))
        {
            ...
            SPU_WriteByte(addr, SPU_ReadByte(addr) | 0x80);
        }
...
void InitSequence(Sequence* seq, u32 sbnk)
{
    ...
    if (SharedMem)
    {
        ...
        _MMU_write32<ARMCPU_ARM7>(seqdata+0x40, 0);
        for (int i = 0; i < 16; i++)
        {
            _MMU_write16<ARMCPU_ARM7>(seqdata+0x20 + (i<<1), 0xFFFF);
        }
    }
}
...
void PrepareSequence(int id, u32 notedata, u32 noteoffset, u32 sbnk)
{
    ...
    u8 firstcmd = _MMU_read08<ARMCPU_ARM7>(track0->CurNoteAddr);
    if (firstcmd == 0xFE)
    {
        // multi track setup
        ...
        u16 mask = _MMU_read08<ARMCPU_ARM7>(track0->CurNoteAddr++);
        mask |= (_MMU_read08<ARMCPU_ARM7>(track0->CurNoteAddr++) << 8);
...
void ProcessCommands()
{
    ...
    while (!CmdQueue.IsEmpty())
    {
        ...
        for (;;)
        {
            ...
            u32 args[4];
            for (u32 i = 0; i < 4; i++)
                args[i] = _MMU_read32<ARMCPU_ARM7>(cmdbuf+0x8 + (i*4));
            ...
            case 0xC: // start
                {
                    ...
                    for (int i = 0; i < 16; i++)
                    {
                        ...
                        u8 cnt = SPU_ReadByte(0x04000403 + (i*16));
                        cnt |= 0x80;
                        SPU_WriteByte(0x04000403 + (i*16), cnt);
                    }

                    mask_cap &= 0x3;
                    if (mask_cap == 0x3)
                    {
                        u16 cnt = SPU_ReadWord(0x04000508);
                        cnt |= 0x8080;
                        SPU_WriteWord(0x04000508, cnt);
                    }
                    else if (mask_cap == 0x1)
                    {
                        u8 cnt = SPU_ReadByte(0x04000508);
                        cnt |= 0x80;
                        SPU_WriteByte(0x04000508, cnt);
                    }
                    else if (mask_cap == 0x2)
                    {
                        u8 cnt = SPU_ReadByte(0x04000509);
                        cnt |= 0x80;
                        SPU_WriteByte(0x04000509, cnt);
                    }
            ...
            case 0xD: // stop
                {
                    ...
                    if (mask_cap & 0x1)
                    {
                        SPU_WriteByte(0x04000508, 0);
                    }
                    if (mask_cap & 0x2)
                    {
                        SPU_WriteByte(0x04000509, 0);
                    }
            ...
            case 0x11: // setup capture
                {
                    ...
                    SPU_WriteByte(0x04000508+num, cnt);
                    SPU_WriteLong(0x04000510+(num*8), dstaddr);
                    SPU_WriteWord(0x04000514+(num*8), len);
                }
                break;
            ...
            case 0x16: // set surround decay
                {
                    ...
                    for (int i = 0; i < 16; i++)
                    {
                        ...
                        u8 pan = SPU_ReadByte(chanaddr+0x02);
                        int vol = CalcChannelVolume(ChanVolume[i], pan);
                        SPU_WriteByte(chanaddr+0x00, vol);
                    }
                }
                break;
            ...
            case 0x17: // set master volume
                {
                    SPU_WriteByte(0x04000500, args[0] & 0xFF);
                }
                break;
            ...
            case 0x18: // set master pan
                {
                    ...
                    if (MasterPan >= 0)
                    {
                        ...
                        for (int i = 0; i < 16; i++)
                        {
                            SPU_WriteByte(0x04000402 + (i*16), pan);
                        }
                    }
                    else
                    {
                        for (int i = 0; i < 16; i++)
                        {
                            SPU_WriteByte(0x04000402 + (i*16), ChanPan[i]);
                        }
                    }
                }
                break;
...
bool ReadInstrument(u32 sbnk, int index, int tune, u8* out)
{
    ...
    u32 numinstr = _MMU_read32<ARMCPU_ARM7>(sbnk + 0x38);
    if ((u32)index >= numinstr) return false;

    u32 val = _MMU_read32<ARMCPU_ARM7>(sbnk + 0x3C + (index << 2));
    out[0] = val & 0xFF;
    if (out[0] >= 1 && out[0] <= 5)
    {
        u32 addr = sbnk + (val >> 8);
        for (int i = 0; i < 5; i++)
        {
            *(u16*)&out[2 + (i << 1)] = _MMU_read16<ARMCPU_ARM7>(addr + (i << 1));
        }
        return true;
    }
    else if (out[0] == 16)
    {
        u32 addr = sbnk + (val >> 8);
        u8 lower = _MMU_read08<ARMCPU_ARM7>(addr);
        u8 upper = _MMU_read08<ARMCPU_ARM7>(addr + 1);
        ...
        for (int i = 0; i < 6; i++)
        {
            *(u16*)&out[(i << 1)] = _MMU_read16<ARMCPU_ARM7>(addr + (i << 1));
        }
        return true;
    }
    else if (out[0] == 17)
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            u8 val = _MMU_read08<ARMCPU_ARM7>(addr + i);
    ...
    case 4:
        {
            ...
            else
            {
                ...
                u32 swar = _MMU_read32<ARMCPU_ARM7>(sbnk + 0x18 + (swar_num << 3));
                if (!swar) return false;

                u32 num_samples = _MMU_read32<ARMCPU_ARM7>(swar + 0x38);
                if (swav_num >= num_samples) return false;

                swav = _MMU_read32<ARMCPU_ARM7>(swar + 0x3C + (swav_num << 2));
            ...
            chan->Type = 0; // wave
            chan->DataFormat = _MMU_read08<ARMCPU_ARM7>(swav+0x00);
            chan->Repeat = _MMU_read08<ARMCPU_ARM7>(swav+0x01);
            chan->SampleRate = _MMU_read16<ARMCPU_ARM7>(swav+0x02);
            chan->SwavFrequency = _MMU_read16<ARMCPU_ARM7>(swav+0x04);
            chan->LoopPos = _MMU_read16<ARMCPU_ARM7>(swav+0x06);
            chan->Length = _MMU_read32<ARMCPU_ARM7>(swav+0x08);
...
u32 ReadNoteOpParam(Track* track, Sequence* seq, int type)
{
    ...
    case 0: // 8-bit value
        {
            u8 val = _MMU_read08<ARMCPU_ARM7>(track->CurNoteAddr++);
            return val;
        }

    case 1: // 16-bit value
        {
            u32 val = _MMU_read08<ARMCPU_ARM7>(track->CurNoteAddr++);
            val |= (_MMU_read08<ARMCPU_ARM7>(track->CurNoteAddr++) << 8);
            return val;
        }
    ...
    case 2: // variable length
        {
            ...
            for (;;)
            {
                u8 byte = _MMU_read08<ARMCPU_ARM7>(track->CurNoteAddr++);
                val = (val << 7) | (byte & 0x7F);
                if (!(byte & 0x80)) break;
            }
            return val;
    ...
    case 3: // based on counter
        {
            s16 val1 = _MMU_read08<ARMCPU_ARM7>(track->CurNoteAddr++);
            val1 |= (_MMU_read08<ARMCPU_ARM7>(track->CurNoteAddr++) << 8);
            s16 val2 = _MMU_read08<ARMCPU_ARM7>(track->CurNoteAddr++);
            val2 |= (_MMU_read08<ARMCPU_ARM7>(track->CurNoteAddr++) << 8);
    ...
    case 4: // variable in sharedmem
        {
            u8 idx = _MMU_read08<ARMCPU_ARM7>(track->CurNoteAddr++);
            u32 addr = GetNoteParamAddr(seq, idx);
            if (addr)
            {
                u32 val = _MMU_read16<ARMCPU_ARM7>(addr);
...
int UpdateTrack(Track* track, Sequence* seq, int id, bool process)
{
    ...
    while (!track->RestCounter)
    {
        ...
        u8 note_op = _MMU_read08<ARMCPU_ARM7>(track->CurNoteAddr++);
        if (note_op == 0xA2)
        {
            note_op = _MMU_read08<ARMCPU_ARM7>(track->CurNoteAddr++);
        ...
        if (!(note_op & 0x80))
        {
            ...
            u8 speed = _MMU_read08<ARMCPU_ARM7>(track->CurNoteAddr++);
            ...
            case 0x90:
                {
                    ...
                    case 0x93: // setup track
                        {
                            u8 tnum = _MMU_read08<ARMCPU_ARM7>(track->CurNoteAddr++);
                            u32 trackaddr = _MMU_read08<ARMCPU_ARM7>(track->CurNoteAddr++);
                            trackaddr |= (_MMU_read08<ARMCPU_ARM7>(track->CurNoteAddr++) << 8);
                            trackaddr |= (_MMU_read08<ARMCPU_ARM7>(track->CurNoteAddr++) << 16);
                            if (!cond) break;
                    ...
                    case 0x94: // jump
                        {
                            u32 jumpaddr = _MMU_read08<ARMCPU_ARM7>(track->CurNoteAddr++);
                            jumpaddr |= (_MMU_read08<ARMCPU_ARM7>(track->CurNoteAddr++) << 8);
                            jumpaddr |= (_MMU_read08<ARMCPU_ARM7>(track->CurNoteAddr++) << 16);
                            if (!cond) break;
            ...
            case 0xB0:
                {
                    ...
                    {
                    case 0xB0: // set
                        _MMU_write16<ARMCPU_ARM7>(paramaddr, param);
                        break;
                    case 0xB1: // add
                        _MMU_write16<ARMCPU_ARM7>(paramaddr, (s16)_MMU_read16<ARMCPU_ARM7>(paramaddr) + param);
                        break;
                    case 0xB2: // sub
                        _MMU_write16<ARMCPU_ARM7>(paramaddr, (s16)_MMU_read16<ARMCPU_ARM7>(paramaddr) - param);
                        break;
                    ...
                    case 0xB6: //
                        {
                            ...
                            if (neg) val = -val;
                            _MMU_write16<ARMCPU_ARM7>(paramaddr, val);
                        }
                        break;
                    case 0xB8: // compare ==
                        track->StatusFlags &= ~(1<<6);
                        if ((s16)_MMU_read16<ARMCPU_ARM7>(paramaddr) == param)
                            track->StatusFlags |= (1<<6);
                        break;
                    case 0xB9: // compare >=
                        track->StatusFlags &= ~(1<<6);
...
void UpdateSequence(Sequence* seq)
{
    ...
    if (SharedMem)
    {
        ...
        _MMU_write32<ARMCPU_ARM7>(addr, _MMU_read32<ARMCPU_ARM7>(addr) + i);
    }
...
void ReportHardwareStatus()
{
    ...
    _MMU_write16<ARMCPU_ARM7>(SharedMem+0x08, chanmask);
    _MMU_write16<ARMCPU_ARM7>(SharedMem+0x0A, capmask);
}
